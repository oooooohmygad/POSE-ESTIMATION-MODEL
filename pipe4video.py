...
def process_frame(frame, timestemp):
    ...
    detection_result = detector.detect_for_video(mp_image, timestemp)

    # STEP 5: Draw pose landmarks on the input image.
    landmarks = detection_result.pose_landmarks
    pose_landmarks = landmark_pb2.NormalizedLandmarkList()
    pose_landmarks_force = landmark_pb2.NormalizedLandmarkList()
    if len(landmarks) == 0:
        return frame
    landmarks_force = copy.deepcopy(landmarks)
    for     i in range(33):
        landmarks_force[0][i].visibility = 1
    # 注意2 landmarks本身是识别多个人的对话框，所以需要遍历，这里只有一个人，所以直接取第1个，即landmarks[0]
    pose_landmarks_force.landmark.extend([
    landmark_pb2.NormalizedLandmark(x=landmark.x, y=landmark.y, z=landmark.z,visibility=landmark.visibility) for landmark in landmarks_force[0]
    ])
    ...
    mp_drawing.draw_landmarks(annotated_image,pose_landmarks_force,myPose.POSE_CONNECTIONS, landmark_drawing_spec=mp_drawing_styles
                .get_default_pose_landmarks_style())
    mp_drawing.draw_landmarks(annotated_image,pose_landmarks,myPose.POSE_CONNECTIONS, DrawingSpec_point, DrawingSpec_line)
